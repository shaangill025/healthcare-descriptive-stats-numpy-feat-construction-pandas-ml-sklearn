...
def logistic_regression_pred(X_train, Y_train, X_test):
	...
	log_reg_pred_model = LogisticRegression()
	log_reg_pred_model.fit(X_train, Y_train)
	return log_reg_pred_model.predict(X_test)
...
def svm_pred(X_train, Y_train, X_test):
	...
	#use default params for the classifier
	np.random.seed(RANDOM_STATE)
	svm_pred_model = LinearSVC()
	svm_pred_model.fit(X_train, Y_train)
	return svm_pred_model.predict(X_test)
...
def decisionTree_pred(X_train, Y_train, X_test):
	...
	np.random.seed(RANDOM_STATE)
	dec_tree_pred_model = DecisionTreeClassifier(max_depth=5)
	dec_tree_pred_model.fit(X_train, Y_train)
	return dec_tree_pred_model.predict(X_test)
